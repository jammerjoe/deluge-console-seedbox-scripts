...
seeded_for_days=$3 ## the number of days back (files) to grep for active torrents from $seedingdir.  used to prevent deletion of popular torrents
...
list_of_recent_seeding_files=`find . -type f -mtime -$seeded_for_days -printf "%f\n" | grep seeded` ## active seeding log files from the last 10 days
...
then
  ...
  while read -r linex; do  ## for each line in the in the list of torrents to delete
    ...
    namex=`echo $linex  | sed s/'\w*$'// | awk '{$1=$1};1'`  ## split the line and get the filename of the torrent
    ...
    case "$tags" in
      *autodl*|*delete*)  ## if the torrent had a label that was either *autodl* or *delete* then 
        case $list_of_recently_active in *"$id"*) #... ;; esac
	...
	*)
          ...
          mvfile=$(printf '%q' "$delugeautoadddownloadloc/$namex") # the printf %q adds escape chars to the special chars in the filename
          mv "$mvfile" "$delugedirdelete"  ## move the files/directories to a location to be deleted later so we dont have to use the dredded rm -Rf in a script that may be blank (ie. deleting your whole directory from whereever this is running)  Setup a crontab job to delete the files from the delete directory
	  ;;
	esac
      ;;
